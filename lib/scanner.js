...
// A typed slash command arrives as a user line whose content STARTS with the
// command wrapper: <command-name> plus <command-message>, in either order
// (the order has drifted across Claude Code versions), optionally with
// <command-args>. Requiring a wrapper tag at the very start keeps prompts
// that merely quote the tags (docs, discussions about the format) from
// counting. The name must look like a real skill id: letters/digits first,
// then word chars, dots, colons for plugin:skill, or hyphens.
const CMD_WRAPPER_START_RE = /^\s*<command-(?:name|message)>/;
const CMD_NAME_TAG_RE = /<command-name>\s*\/?([A-Za-z0-9][\w.:-]*)\s*<\/command-name>/;
const CMD_ARGS_TAG_RE = /<command-args>([\s\S]*?)<\/command-args>/;

function parseTypedCommand(rawText) {
  if (!rawText || !CMD_WRAPPER_START_RE.test(rawText)) return null;
...
// Built-in Claude Code commands that share the <command-name> wrapper but are
// not skills. Kept to commands that were never skills; names that ship as
// skills in current Claude Code (init, code-review, security-review) are
// deliberately absent so they count.
const BUILTIN_COMMANDS = new Set([
  'add-dir', 'agents', 'bashes', 'bug', 'clear', 'compact', 'config',
  'context', 'cost', 'doctor', 'effort', 'exit', 'export', 'help', 'hooks',
  'ide', 'install-github-app', 'keybindings', 'login', 'logout', 'mcp',
  'memory', 'migrate-installer', 'model', 'output-style', 'permissions',
  'plugin', 'pr-comments', 'privacy-settings', 'quit', 'release-notes',
  'reload-plugins', 'reload-skills', 'rename', 'resume', 'review', 'rewind',
  'sandbox', 'settings', 'status', 'statusline', 'terminal-setup', 'theme',
  'todos', 'upgrade', 'usage', 'vim',
]);

function matchTypedSkill(rawText) {
  const cmd = parseTypedCommand(rawText);
  if (!cmd || BUILTIN_COMMANDS.has(cmd.name.toLowerCase())) return null;
  return cmd.name;
}

// Global skill-invocation dedup, mirroring the usage claim in buildIndex:
// forked and resumed session files copy history verbatim, so the same
// invocation (same tool_use id or line uuid) appears in several files.
// Walked oldest-first, the original file claims each cid and a fork keeps
// only its new invocations. Rewrites each session's skillRecords in place to
// the compact shipped shape (cid dropped).
function claimSkillRecords(sessionsOldestFirst) {
  const claimed = new Set();
  for (const s of sessionsOldestFirst) {
    const kept = [];
    for (const r of (s.skillRecords || [])) {
      if (!r || !r.cid || !r.skill || claimed.has(r.cid)) continue;
      claimed.add(r.cid);
      kept.push({ skill: r.skill, by: r.by, d: r.d, u: r.u });
    }
    s.skillRecords = kept;
  }
}

// Read-only inventory of installed skills: ~/.claude/skills/<name>/SKILL.md
// (entries are often symlinks) plus plugin skills discovered through the
// installed-plugins manifest. Tolerant: anything unreadable is skipped.
async function scanSkillInventory() {
  const out = [];
  const seen = new Set();
  const home = os.homedir();
  const add = (id, source) => {
    if (id && !seen.has(id)) { seen.add(id); out.push({ id, source }); }
  };
  let entries = [];
  const skillsDir = path.join(home, '.claude', 'skills');
  try { entries = await fsp.readdir(skillsDir, { withFileTypes: true }); } catch {}
  for (const e of entries) {
    if (e.name.startsWith('.')) continue;
    if (!e.isDirectory() && !e.isSymbolicLink()) continue;
    try {
      await fsp.access(path.join(skillsDir, e.name, 'SKILL.md'));
      add(e.name, 'user');
    } catch {}
  }
  try {
    ...
    const plugins = manifest && typeof manifest.plugins === 'object' && manifest.plugins ? manifest.plugins : {};
    for (const [key, installs] of Object.entries(plugins)) {
      const pluginName = String(key).split('@')[0];
      if (!pluginName || !Array.isArray(installs) || !installs.length) continue;
      ...
      let dirs = [];
      try { dirs = await fsp.readdir(skillsRoot, { withFileTypes: true }); } catch { continue; }
      for (const d of dirs) {
        if (d.name.startsWith('.')) continue;
        if (!d.isDirectory() && !d.isSymbolicLink()) continue;
        try {
          await fsp.access(path.join(skillsRoot, d.name, 'SKILL.md'));
          add(pluginName + ':' + d.name, 'plugin');
        } catch {}
      }
    }
  } catch {}
...
async function extractMeta(filePath) {
  ...
  const meta = {
    ...
    usageRecords: [],
    // skill invocations seen in this file: { cid, skill, by, d, u } where cid
    // is the global claim id (tool_use id or line uuid), by is 'claude' or
    // 'typed', d the day key, u the line uuid for the tape jump. Deduped
    // globally in buildIndex, exactly like usageRecords.
    skillRecords: [],
    sidechainEvents: 0,
    events: 0,
  };
  ...
  for await (const line of rl) {
    ...
    if (isRealUserMessage(obj)) {
      ...
      // Typed skill invocation: the command wrapper must be read off the RAW
      // text, because cleanPrompt strips <command-name> tags. Compaction
      // summaries quote command wrappers verbatim, so they are excluded.
      if (obj.uuid && !obj.isCompactSummary) {
        const skill = matchTypedSkill(raw);
        const d = obj.timestamp ? dayKey(obj.timestamp) : null;
        if (skill && d) {
          meta.skillRecords.push({ cid: obj.uuid, skill, by: 'typed', d, u: obj.uuid });
        }
      }
      ...
      if (Array.isArray(m.content)) {
        for (let bi = 0; bi < m.content.length; bi++) {
          ...
          if (block && block.type === 'tool_use') {
            ...
            // invocations are excluded by construction.
            if (name === 'Skill' && obj.uuid) {
              const skill = block.input && typeof block.input.skill === 'string'
                ? block.input.skill.trim() : '';
              const d = obj.timestamp ? dayKey(obj.timestamp) : null;
              const cid = block.id || obj.uuid + '#' + bi;
              if (skill && d) {
                meta.skillRecords.push({ cid, skill, by: 'claude', d, u: obj.uuid });
              }
            }
          }
        }
...
async function buildIndex(cache, onProgress) {
  // Installed-skill inventory: read-only, once per build, feeds the Skills
  // panel's never-fired / idle flags.
  const skills = await scanSkillInventory();
  let dirents;
  try {
    dirents = await fsp.readdir(PROJECTS_DIR, { withFileTypes: true });
  } catch {
    return { projects: [], sessions: [], skills };
  }
  ...
  // Skill invocations get the same global claim, over the same deterministic
  // oldest-first order. Rewrites s.skillRecords only; the cached meta keeps
  // its full per-file records.
  claimSkillRecords(byAge);
...
module.exports = {
  ...
  claimSkillRecords, scanSkillInventory, matchTypedSkill,
};
