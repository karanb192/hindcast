...
async function extractMeta(filePath) {
  ...
  for await (const line of rl) {
    ...
    if (isRealUserMessage(obj)) {
      const text = cleanPrompt(textOfContent(obj.message.content));
      if (text) {
        meta.userTurns++;
...
async function buildIndex(cache, onProgress) {
  ...
  const byAge = [...sessions].sort((a, b) => (a.firstTs || Infinity) - (b.firstTs || Infinity));
...
async function parseSession(filePath, opts = {}) {
  ...
  for (const obj of objs) {
    ...
    if (obj.type === 'user' && obj.message) {
      ...
      const text = cleanPrompt(textOfContent(c));
...
module.exports = { PROJECTS_DIR, buildIndex, parseSession, listSubagents, cleanPrompt, extractMeta, foldSubagentUsage, aggregateRecords };
