...
const CMD_TOKEN_RE = /<(\/?)command-[a-z-]+>/g;

// Residue of the line once every <command-*> wrapper is cut: each opener
// swallows through the NEXT command closer of any name; unpaired tags stay.
// One token scan plus slices keeps this linear in the line length; a lazy
// wrapper regex re-scanned to the end of the line for every unclosed
// opener, which let one hostile pasted line stall the main process.
function stripCommandWrappers(text) {
  ...
  let m;
  while ((m = CMD_TOKEN_RE.exec(text))) {
    if (!m[1]) {
      if (openAt === -1) openAt = m.index;
    } else if (openAt !== -1) {
      out += text.slice(keepFrom, openAt);
      keepFrom = m.index + m[0].length;
      openAt = -1;
    }
  }
...
function parseTypedCommand(rawText) {
  ...
  if (!m || stripCommandWrappers(rawText).trim()) return null;
...
// 'builtin'), u line uuid, ts epoch ms. The uuid keys the global claim in
// buildIndex (with the name, see claimSessionSkills) and is the tape-jump
// target; a line missing its uuid or a parseable timestamp can be neither
// claimed nor placed in time, so it is dropped, the same rule
// pushUsageRecord applies.
...
// The claim key is uuid plus name, not uuid alone, so two well-formed
// Skill blocks sharing one assistant line both count; current writers land
// one block per line, but multi-block lines are legal everywhere else.
...
async function buildIndex(cache, onProgress) {
  ...
  // by line uuid plus name: forks copy the invocation lines verbatim too.
